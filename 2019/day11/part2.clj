...
(defn run-robot []
  (loop [computer (create-computer (slurp "input.txt") [])
         robot {:pos [0 0] :direction 0 :paint  {[0 0] 1}}]
    (if (:halted computer)
      (:paint robot)
...
(defn render-paint [paint]
  (let [min-x (apply min (map first (keys paint)))
        max-x (apply max (map first (keys paint)))
        min-y (apply min (map second (keys paint)))
        max-y (apply max (map second (keys paint)))
        lines (map (fn [y] 
