...
(defn steps-until [steps intersection]
  (->> (range (count steps))
       (filter #(= intersection (get steps %)))
       (first)
       inc))

(let [grid [[0 0] []]
      [moves1 moves2] (parse-input input)
      [_ steps1] (trace-moves grid moves1)
      [_ steps2] (trace-moves grid moves2)
      intersections (clojure.set/intersection (set steps1) (set steps2))]
  (->> intersections
       (map (fn [intersection]
              (+ (steps-until steps1 intersection)
                 (steps-until steps2 intersection))))
       (apply min)))
